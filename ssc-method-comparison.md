...
Ontology subclass expansion is an NDE/MCP-only capability (`get_descendants`), and the SSc **D run actually used it** (the stream shows it looked up SSc's URI, enumerated subtypes, and excluded morphea). Expanding systemic sclerosis (MONDO:0005100) to its **7 MONDO subclasses** (diffuse/limited cutaneous SSc, diffuse scleroderma, CREST, …) raises the raw count **167 → 170 (+1.8%, +3 datasets)**.

Curating those 3: **1 is a genuine valid recovery** — **GSE9285** ("Gene Expression Profiling of Scleroderma Skin", 34 SSc subjects; a *canonical* SSc skin dataset the exact-"systemic sclerosis" query missed, because NDE tags it "diffuse scleroderma"/"limited scleroderma" not the parent). The other 2 are false positives: **GSE12493** (in-vitro dermal fibroblasts + TGF-β treatment) and **GSE86212** (a **CHARGE-syndrome iPSC neural-crest** study mis-tagged "CREST syndrome" — an acronym collision).

So for SSc, **subclass expansion adds +1.8% raw recall and 1 valid dataset (GSE9285)** — small but real, and notably it recovered a dataset that even web's curated set relied on. This is the meaningful contrast with glaucoma (**+19% raw but 0 valid**) and DN (MONDO leaf, 0): the payoff of subclass expansion depends entirely on whether a disease's subtypes host real patient cohorts (SSc: one does; glaucoma: none do).
